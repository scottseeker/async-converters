...
function calcIv(stat: number, baseStat: number, level: number, isHp: boolean) {
  if (isHp) {
    return Math.round(((stat - level - 10) * 100 / level) - 2 * baseStat);
  }
...
export default function PokemonIv() {
  ...
  return (
    <ConverterShell title="Pokémon IV Estimator" description="Estimate a Pokémon's Individual Values (IVs) from its stat, base stat, and level." category="gaming">
      <div className={styles.form}>
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
          <label style={{ display: 'flex', gap: '0.4rem', alignItems: 'center', cursor: 'pointer' }}>
            <input type="checkbox" checked={isHp} onChange={e => setIsHp(e.target.checked)} /> HP stat
          </label>
        </div>
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          <div className={styles.field} style={{ flex: 1, minWidth: 100 }}>
            <label>Actual stat</label>
        ...
        {clamped !== null && (
          <div className={styles.stats}>
            <div className={styles.stat}>
              <div className={styles.statNum} style={{ color: clamped >= 28 ? 'var(--accent)' : clamped >= 20 ? undefined : '#e55' }}>{clamped} / 31</div>
              <div className={styles.statLabel}>Estimated IV</div>
            </div>
            <div className={styles.stat}>
              <div className={styles.statNum}>{clamped >= 28 ? '⭐ Excellent' : clamped >= 20 ? 'Good' : clamped >= 10 ? 'OK' : 'Poor'}</div>
              <div className={styles.statLabel}>Rating</div>
            </div>
          </div>
        )}
        <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Note: EVs and nature are not accounted for in this simplified estimate.</p>
      </div>
    </ConverterShell>
  );
}
